...
class ListViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let simpsonInfo = sender as? SimpsonCharacter else { return }
...
extension ListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let data = viewModel.getSimpsonData(index: indexPath.row)
        self.performSegue(withIdentifier: "OpenDetails", sender: data)
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let title = viewModel.getSimpsonTitle(index: indexPath.row)
